...
def convert_text_to_feature(A, B1, B2, mc_tokenizer, max_seq_length=70):
    sent1 = mc_tokenizer.tokenize(A.replace("_", B1))
    sent2 = mc_tokenizer.tokenize(A.replace("_", B2))
    _truncate_seq_pair(sent1, [], max_seq_length - 2)
    _truncate_seq_pair(sent2, [], max_seq_length - 2)
    sent1 = [mc_tokenizer.cls_token] + sent1 + [mc_tokenizer.sep_token]
    sent2 = [mc_tokenizer.cls_token] + sent2 + [mc_tokenizer.sep_token]
    ...
    input_ids = torch.tensor([input_ids_1, input_ids_2],
                             dtype=torch.long).to(device).view(1, 2, -1)
    input_mask = torch.tensor([input_mask_1, input_mask_2],
                              dtype=torch.long).to(device).view(1, 2, -1)
...
with torch.no_grad():
    for example in tqdm(data):
        ...
        for pos in candidate_pos:
            ...
            input_ids, input_mask = convert_text_to_feature(
                q_deleted, example[3], example[4], mc_tokenizer)
            output = mc_model(input_ids=input_ids, attention_mask=input_mask)
            logits = output[0]
        ...
        for p in sorted_candidate_pos:
            ...
            for (syn_w, similarity) in word_vector.similar_by_word(w.text,
                ...
                if semantic_similarity < 0.7:
                    ...
                    continue
                input_ids, input_mask = convert_text_to_feature(
                    new_q, example[3], example[4], mc_tokenizer)
                output = mc_model(input_ids=input_ids, attention_mask=input_mask)
                logits = output[0]
