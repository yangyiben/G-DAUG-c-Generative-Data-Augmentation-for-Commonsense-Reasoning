...
def convert_text_to_feature(A, B1, B2, B3, B4,B5, mc_tokenizer, max_seq_length=70):
    Q = mc_tokenizer.tokenize('Q: ' + A)
    ending1 = mc_tokenizer.tokenize('A: ' + B1)
    ending2 = mc_tokenizer.tokenize('A: ' + B2)
    ending3 = mc_tokenizer.tokenize('A: ' + B3)
    ending4 = mc_tokenizer.tokenize('A: ' + B4)
    ending5 = mc_tokenizer.tokenize('A: ' + B5)
    ...
    _truncate_seq_pair(Q, ending5, max_seq_length - 3)
    ...
    sent5 = [mc_tokenizer.cls_token] + Q + [mc_tokenizer.sep_token] + ending5 + [mc_tokenizer.sep_token]
    ...
    input_ids_5 = mc_tokenizer.convert_tokens_to_ids(sent5)
    input_mask_5 = [1] * len(input_ids_5)
    max_len = max(len(input_ids_1), len(input_ids_2), len(input_ids_3), len(input_ids_4), len(input_ids_5))
    ...
    pad_length_5 = max_len - len(input_ids_5)
    ...
    input_ids_5 = input_ids_5 + [mc_tokenizer.pad_token_id] * pad_length_5
    input_mask_5 = input_mask_5 + [0] * pad_length_5
    input_ids = torch.tensor([input_ids_1, input_ids_2, input_ids_3, input_ids_4, input_ids_5],
                             dtype=torch.long).to(device).view(1, 5, -1)
    input_mask = torch.tensor([input_mask_1, input_mask_2, input_mask_3, input_mask_4, input_mask_5],
                              dtype=torch.long).to(device).view(1, 5, -1)
...
with torch.no_grad():
    for example in tqdm(data):
        ...
        input_ids, input_mask = convert_text_to_feature(
            q, example[3], example[4],example[5], example[6],example[7], mc_tokenizer)
        ...
        for pos in candidate_pos:
            ...
            input_ids, input_mask = convert_text_to_feature(
                q_deleted, example[3], example[4],example[5], example[6],example[7], mc_tokenizer)
            output = mc_model(input_ids=input_ids, attention_mask=input_mask)
            logits = output[0]
        ...
        for p in sorted_candidate_pos:
            ...
            for (syn_w, similarity) in word_vector.similar_by_word(w.text,
                ...
                if semantic_similarity < 0.7:
                    ...
                    continue
                input_ids, input_mask = convert_text_to_feature(
                    new_q, example[3], example[4],example[5], example[6],example[7],mc_tokenizer)
                output = mc_model(input_ids=input_ids, attention_mask=input_mask)
                logits = output[0]
