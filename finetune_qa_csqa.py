...
from modeling import GenerativeGPT2QA2
...
MODEL_CLASSES = {
    'gpt2': (GPT2Config, GenerativeGPT2QA2, GPT2Tokenizer),
}
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.multi_task and not (args.mc):
                outputs, mc_loss = model(**mc_inputs, lm=True, mc=True,num_labels=batch[0].size(1))
                lm_loss = outputs.sum() / ((outputs != 0).float().sum())
                #print(lm_loss)
                #print(mc_loss)
                loss = lm_loss * 1 + mc_loss * 10
            elif args.mc:
                mc_loss = model(**mc_inputs, lm=False, mc=True,num_labels=batch[0].size(1))
                loss = mc_loss[0]
            else:
                outputs = model(**inputs, lm=True, mc=False,num_labels=batch[0].size(1))
                outputs = outputs[0]
                lm_loss = outputs.sum() / ((outputs != 0).float().sum())
                loss = lm_loss
...
def evaluate(args,
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
            ...
            with torch.no_grad():
                ...
                if mc:
                    logits_mc_outputs = model(**mc_inputs,
                                              lm=True,
                                              mc=args.multi_task,num_labels=batch[0].size(1))
                    mc_outputs = logits_mc_outputs[0]
                    ...
                    mc_score_normalized = (mc_score_normalized * -1).view(
                        -1,  batch[0].size(1)).data.cpu().numpy()
                    if args.multi_task:
                        mc_logits = logits_mc_outputs[1]
                        mc_head_prob = F.softmax(mc_logits,
                                                 -1).data.cpu().numpy()
                        mc_logits = mc_logits.data.cpu().numpy()
                    ...
                    if preds is None:
                        ...
                        if args.multi_task:
                            preds_mc_head = mc_logits
                            preds_ensemble = mc_prob + mc_head_prob
                    else:
                        ...
                        preds_normalized = np.append(preds_normalized,
                                                     ...
                                                     axis=0)
                        if args.multi_task:
                            preds_mc_head = np.append(preds_mc_head,
                                                      mc_logits,
                                                      axis=0)
                            preds_ensemble = np.append(mc_prob + mc_head_prob,
                                                       preds_ensemble,
                                                       axis=0)
                ...
                if generation:
                    #sep_id = tokenizer.convert_tokens_to_ids(["[EOQ]"])[0]
        ...
        if mc:
            ...
            preds_normalized = randargmax(preds_normalized, axis=1)
            acc = simple_accuracy(preds, out_label_ids)
            acc_normalized = simple_accuracy(preds_normalized, out_label_ids)
            if args.multi_task:
                preds_mc_head = randargmax(preds_mc_head, axis=1)
                acc_mc_head = simple_accuracy(preds_mc_head, out_label_ids)
                preds_ensemble = randargmax(preds_ensemble, axis=1)
                acc_ensemble = simple_accuracy(preds_ensemble, out_label_ids)
                result = {
                    "eval_ppl": eval_ppl,
                    "eval_avg_ppl": eval_avg_ppl,
                    "eval_acc": acc,
                    "eval_acc_normalized": acc_normalized,
                    "eval_acc_mc_head": acc_mc_head,
                    "eval_acc_ensemble": acc_ensemble
                }
            else:
                result = {
                    "eval_ppl": eval_ppl,
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    cached_features_file = os.path.join(
        args.data_dir, 'cached_{}_{}_{}_{}_{}_{}_{}v2'.format(
            ...
            "generative", str(task), "reverse_" if args.reverse else ""))
...
def main():
    ...
    #tokenizer.add_special_tokens({'additional_special_tokens': ["[EOQ]"]})
    ...
    #print(model.tokenizer.all_special_tokens)
    #print(model.tokenizer._convert_token_to_id("[PAD]") )
    #print(model.tokenizer._convert_token_to_id("[CLS]") )
    #print(model.tokenizer.all_special_ids)
    #x = input("stop")
