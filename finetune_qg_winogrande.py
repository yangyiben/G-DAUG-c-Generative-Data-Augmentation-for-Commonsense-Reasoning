...
from modeling import GenerativeGPT2QGWinogrande

from transformers import AdamW, WarmupLinearSchedule, WarmupConstantSchedule

from winogrande_data_utils import (convert_examples_to_generative_features, processors)
...
MODEL_CLASSES = {
    'gpt2': (GPT2Config, GenerativeGPT2QGWinogrande, GPT2Tokenizer),
}
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #outputs = outputs[:,1:]
            loss = outputs.sum() / ((outputs != 0).float().sum())
            #loss = mc_loss
            if args.n_gpu > 1:
                loss = loss.mean(
...
def evaluate(args, model, tokenizer, prefix="", test=False, generation=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if generation:
            output_file = open(
                os.path.join(eval_output_dir, "mle_generated_question.tsv"),
                'w')
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                if generation:
                    sentence = model.generate(inputs['input_ids'].size(0),
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    cached_features_file = os.path.join(
        args.data_dir, 'cached_{}_{}_{}_{}_{}_{}'.format(
            cached_mode,
            list(filter(None, args.model_name_or_path.split('/'))).pop(),
            str(args.input_max_seq_length), str(args.output_max_seq_length),
            "generativeqg_winogrande", str(task)))
    if os.path.exists(cached_features_file):
        ...
        features = convert_examples_to_generative_features(
            examples,
            label_list,
            args.input_max_seq_length,
            args.output_max_seq_length,
            tokenizer,
